...
import { YouTubersComparison } from "@/components/YouTubersComparison";
import { YourRankCard } from "@/components/YourRankCard";
import { TopPromoCodes } from "@/components/TopPromoCodes";
import { ChartRevenueVsYouTubers } from "@/components/analytics/ChartRevenueVsYouTubers";
import { ChartTrendYouVsAverage } from "@/components/analytics/ChartTrendYouVsAverage";
import { ChartConversionVsYouTubers } from "@/components/analytics/ChartConversionVsYouTubers";
import { ChartPurchasesVsYouTubers } from "@/components/analytics/ChartPurchasesVsYouTubers";
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-[hsl(var(--muted))] sm:text-base">
                Сравнение с другими ютуберами, динамика и конверсии в одном отчёте.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <span className="chip">Сравнение каналов</span>
            <span className="chip">Период: неделя</span>
          </div>
        </motion.header>

        <section className="mb-10" aria-label="Ваше место">
          <YourRankCard />
        </section>

        <section className="mb-10" aria-label="Игроков по доменам">
          <TopPromoCodes />
        </section>

        <section className="mb-10" aria-label="Сравнение со средним">
          <YouTubersComparison />
        </section>

        <section className="mb-10 grid gap-10 lg:grid-cols-2" aria-label="Графики сравнения">
          <ChartRevenueVsYouTubers />
          <ChartTrendYouVsAverage />
          <ChartConversionVsYouTubers />
          <ChartPurchasesVsYouTubers />
        </section>
