...
export function DomainDrawer() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="flex shrink-0 items-center justify-between gap-4 border-b border-[hsl(var(--border))] px-6 py-4">
          ...
          <motion.button
            ...
            onClick={closeDrawer}
            whileTap={{ scale: 0.95 }}
            className="flex h-10 w-10 items-center justify-center rounded-xl bg-[hsl(var(--surface-muted))] text-[hsl(var(--muted))] transition-colors hover:bg-[hsl(var(--surface-muted))]/80 hover:text-[hsl(var(--foreground))]"
            aria-label="Закрыть"
          >
            <X className="h-5 w-5" />
          </motion.button>
...
function SingleDomainContent({ main, current }: { main: DomainStats; current: DomainStats }) {
  ...
  return (
    <motion.div
      ...
      className="space-y-6"
    >
      <div className="flex items-center gap-4 rounded-2xl bg-[#DC2626]/10 p-5 ring-1 ring-[#DC2626]/20">
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-[#DC2626] text-white">
      ...
      {main.subdomains && main.subdomains.length > 0 && (
        <div className="rounded-xl border border-[hsl(var(--border))]/60 bg-[hsl(var(--surface-muted))]/30 p-4">
          ...
          <ul className="space-y-2">
            <li className="flex items-center justify-between rounded-lg bg-[hsl(var(--surface))]/50 px-3 py-2 text-[13px]">
              ...
              <span className="tabular-nums text-[#DC2626]">{formatRubles(main.revenue)} · {main.attractedUsers.toLocaleString("ru-RU")} игроков</span>
            </li>
            {main.subdomains.map((sub) => (
              <li key={sub.domain} className="flex items-center justify-between rounded-lg bg-[hsl(var(--surface))]/50 px-3 py-2 text-[13px]">
                ...
                <span className="tabular-nums text-[#DC2626]">{formatRubles(sub.revenue)} · {sub.attractedUsers.toLocaleString("ru-RU")} игроков</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="rounded-xl border-2 border-[#DC2626]/20 bg-[#DC2626]/5 p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-lg border border-[hsl(var(--border))]/60 bg-[hsl(var(--surface))] p-3">
            ...
            <p className="mt-1 text-lg font-bold tabular-nums text-[#DC2626]">{formatRubles(myStats.revenue)}</p>
          ...
          <div className="rounded-lg border border-[hsl(var(--border))]/60 bg-[hsl(var(--surface))] p-3">
            ...
            <p className="mt-1 text-lg font-bold tabular-nums text-[#DC2626]">{formatRubles(current.revenue)}</p>
          ...
          <div>
            ...
            <div className="h-2 overflow-hidden rounded-full bg-[hsl(var(--surface-muted))]">
              <motion.div
                ...
                className="h-full rounded-full bg-[#DC2626]/80"
              />
            </div>
          </div>
        </div>
...
function StatBlock({
  ...
  return (
    <div className={`rounded-xl border p-4 ${accent ? "border-[#DC2626]/30 bg-[#DC2626]/5" : "border-[hsl(var(--border))]/60 bg-[hsl(var(--surface-muted))]/30"}`}>
      <div className="flex items-center gap-2 text-[12px] font-medium uppercase tracking-wider text-[hsl(var(--muted))]">
        <Icon className="h-4 w-4" strokeWidth={2} />
        {label}
      </div>
      <p className={`mt-2 text-xl font-bold tabular-nums ${accent ? "text-[#DC2626]" : "text-[hsl(var(--foreground))]"}`}>
        {value}
      </p>
    </div>
  );
...
function AllDomainsTable() {
  ...
  return (
    ...
    >
      ...
      <div className="overflow-hidden rounded-xl border border-[hsl(var(--border))]/60">
        <table className="w-full border-collapse text-[13px]">
          <thead>
            <tr className="bg-[hsl(var(--surface-muted))]/60">
          ...
          <tbody>
            {sorted.map((d, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-right font-semibold tabular-nums text-[#DC2626]">
